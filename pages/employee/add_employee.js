...
const AddEmployee = () => {
    ...
    const formik = useFormik({
        initialValues: { name: "", contact: "", email: "", associatedWith: "", password: "", role: "", isAssociated: "yes" },
    ...
    const submit = (values) => {
        const id = employees?.employees[employees?.employees?.length - 1]?.id + 1 || 1;
        const reports = 7
        const clients = 15
        const applications = 4
        const createdBy = "Admin Z"
        dispatch(employeeManagementAction.createEmployee({ ...values, id, reports, applications, createdBy, clients }));
        router.push("/employee/employees");
    };
    const [showPassword, setShowPassword] = useState(false)
    return (
        <div style={{ height: 'auto' }} className={styles.add}>
            <h4 className="mb-5">Add New Employee</h4>
                ...
                <Row className="mt-3">
                    <Form.Group className={`d-flex ${styles.radio_check}`} style={{ color: "black" }}>
                        <Form.Check
                            ...
                            onBlur={formik.handleBlur}
                            checked={formik.values.isAssociated === 'yes'}
                        />
                        <Form.Check
                            ...
                            onBlur={formik.handleBlur}
                            checked={formik.values.isAssociated === 'no'}
